...
export function prospectToRow(p: Prospect, userId: string) {
  return {
    ...
    email: p.email || null,
    rating: p.rating !== undefined && p.rating !== null && !isNaN(Number(p.rating)) ? Number(p.rating) : null,
    review_count: p.reviewCount !== undefined && p.reviewCount !== null && !isNaN(Number(p.reviewCount)) ? Math.round(Number(p.reviewCount)) : null,
    notes: p.notes || null,
    identified: p.identified,
    status: p.status,
    relevance_score: p.relevanceScore !== undefined && p.relevanceScore !== null && !isNaN(Number(p.relevanceScore)) ? Number(p.relevanceScore) : null,
    key_angle: p.keyAngle || null,
    lat: p.lat !== undefined && p.lat !== null && !isNaN(Number(p.lat)) ? Number(p.lat) : null,
    lng: p.lng !== undefined && p.lng !== null && !isNaN(Number(p.lng)) ? Number(p.lng) : null,
